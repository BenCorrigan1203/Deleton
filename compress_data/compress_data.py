"""This script compress the daily data to insert into historical database"""
import os
...
from compress_sql import ADDRESS_SQL, RIDER_SQL, METADATA_SQL, RESISTANCE_SQL
from compress_sql import POWER_SQL, HEART_SQL, RPM_SQL, RIDE_SQL, RIDE_INFO_SQL
from compress_sql import DELETE_META_SQL, DELETE_RIDE_SQL


def get_db_connection(config: dict=os.environ) -> connection:
    """establishes connection to database"""
    try:
        connection_database = psycopg2.connect(user = config["DB_USER"], \
                                      host = config["DB_HOST"], \
...
def insert_resis_power_rpm_heart(conn: connection, meta_data: list[dict]) \
    ...
    for i in range(len(meta_data)):
        ...
        cur.execute(RESISTANCE_SQL, (meta_data[i]["resistance_avg"],meta_data[i]["resistance_max"]\
                                     ,meta_data[i]["resistance_min"],meta_data[i]["resistance_avg"]\
                                        ,meta_data[i]["resistance_max"],meta_data[i]["resistance_min"],))
        resistance_id.append(cur.fetchone()['resistance_id'])

        #Inserting power readings and returning id
        cur.execute(POWER_SQL, (meta_data[i]["power_avg"],meta_data[i]["power_max"]\
                                ,meta_data[i]["power_min"],meta_data[i]["power_avg"]\
                                    ,meta_data[i]["power_max"],meta_data[i]["power_min"],))
        power_id.append(cur.fetchone()['power_id'])

        #Inserting heart readings and returning id
        cur.execute(HEART_SQL, (meta_data[i]["heart_avg"],meta_data[i]["heart_max"]\
                                ,meta_data[i]["heart_min"],meta_data[i]["heart_avg"]\
                                    ,meta_data[i]["heart_max"],meta_data[i]["heart_min"],))
        heart_id.append(cur.fetchone()['heart_rate_id'])


        #Inserting rpm readings and returning id
        cur.execute(RPM_SQL, (meta_data[i]["rpm_avg"],meta_data[i]["rpm_max"]\
                              ,meta_data[i]["rpm_min"],meta_data[i]["rpm_avg"]\
                                ,meta_data[i]["rpm_max"],meta_data[i]["rpm_min"],))
...
def insert_ride_info(conn: connection, meta_data: list[dict], heart_id: list\
                     ,resistance_id: list, power_id: list, rpm_id: list):
    ...
    for i in range(len(meta_data)):
        ...
        cur.execute(RIDE_INFO_SQL, (ride_data[0]['start_time'], ride_data[0]['end_time']\
                                    , ride_data[0]['rider_id'], ride_data[0]['bike_serial']\
                                        , heart_id[i], resistance_id[i], power_id[i] ,rpm_id[i],))
    conn.commit()
    cur.close()
