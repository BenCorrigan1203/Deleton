  ...
  [
    {
      ...
      "logConfiguration": {
        ...
        "options": {
          ...
          "awslogs-group": "${aws_cloudwatch_log_group.c7-deleton-ingestion-watchgroup.name}",
          "awslogs-stream-prefix": "c7-deleton-container"
        }},
      "environment": [
...
resource "aws_ecs_service" "c7-deleton-ingestion-task-service" {
  ...
  #   namespace = aws_service_discovery_http_namespace.c7-deleton-ecs-service-namespace.name
  #   enabled   = true
  #   log_configuration {
...
resource "aws_service_discovery_http_namespace" "c7-deleton-ecs-service-namespace" {
  name        = "c7-deleton-ecs-service-namespace"
  description = "example"
}

resource "aws_cloudwatch_log_group" "c7-deleton-ingestion-watchgroup" {
  name              = "c7-deleton-ingestion-watchgroup"
  retention_in_days = 7
  lifecycle {
...
resource "aws_cloudwatch_log_group" "c7-deleton-api-watchgroup" {
  name              = "c7-deleton-api-watchgroup"
  retention_in_days = 7
  lifecycle {
    prevent_destroy = false
  }
  ...
  [
    {
      ...
      "logConfiguration": {
        ...
        "options": {
          "awslogs-region": "eu-west-2",
          "awslogs-group": "${aws_cloudwatch_log_group.c7-deleton-api-watchgroup.name}",
          "awslogs-stream-prefix": "c7-deleton-container"
        }},
      "environment": [
        {"name": "ACCESS_KEY", "value":   "${var.ACCESS_KEY}"},
...
resource "aws_ecs_service" "c7-deleton-api-task-service" {
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.c7-deleton-api-lb-target-group.arn
...
resource "aws_cloudwatch_log_group" "c7-deleton-live-dash-watchgroup" {
  name              = "c7-deleton-live-dash-watchgroup"
  retention_in_days = 7
  lifecycle {
    prevent_destroy = false
  }
...
resource "aws_cloudwatch_log_stream" "c7-deleton-api-cloudwatch-logstream" {
  name           = "c7-deleton-api-cloudwatch-logstream"
  log_group_name = aws_cloudwatch_log_group.c7-deleton-api-watchgroup.name
}
# Create a cloudwatch log stream for the ingestion
resource "aws_cloudwatch_log_stream" "c7-deleton-ingestion-cloudwatch-logstream" {
  name           = "c7-deleton-ingestion-cloudwatch-logstream"
  log_group_name = aws_cloudwatch_log_group.c7-deleton-ingestion-watchgroup.name
}
# Create a cloudwatch log stream for the live dash
resource "aws_cloudwatch_log_stream" "c7-deleton-live-dash-cloudwatch-logstream" {
  name           = "c7-deleton-live-dash-cloudwatch-logstream"
  log_group_name = aws_cloudwatch_log_group.c7-deleton-live-dash-watchgroup.name
}
...
resource "aws_ecs_task_definition" "c7-deleton-live-dash-task-definition" {
  ...
  [
    {
      "name": "project-container",
      "image": "605126261673.dkr.ecr.eu-west-2.amazonaws.com/c7-deleton-live-dashboard:latest",
      "portMappings": [
      {
        "containerPort": 8080,
        "protocol": "tcp"
      ...
      "logConfiguration": {
        ...
        "options": {
          "awslogs-region": "eu-west-2",
          "awslogs-group": "${aws_cloudwatch_log_group.c7-deleton-live-dash-watchgroup.name}",
          "awslogs-stream-prefix": "c7-deleton-container"
        }},
      "environment": [
        {"name": "ACCESS_KEY", "value":   "${var.ACCESS_KEY}"},
...
resource "aws_ecs_service" "c7-deleton-live-dash-task-service" {
  name            = "c7-deleton-live-dash-task-service"
  cluster         = aws_ecs_cluster.c7-deleton-ecs-cluster.id
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.c7-deleton-live-dash-lb-target-group.arn
    container_name   = "project-container"
    container_port   = 8080
  }
}
...
resource "aws_lb" "c7-deleton-api-lb" {
  name               = "c7-deleton-api-lb"
  load_balancer_type = "application"
  subnets            = ["subnet-0bd43551b596597e1", "subnet-0b265a90c0cadfb99", "subnet-07f982f51c870f9d1"]
  security_groups    = ["sg-01745c9fa38b8ed68"]
}
# Create Load Balancer target group for the API
resource "aws_lb_target_group" "c7-deleton-api-lb-target-group" {
  name        = "c7-deleton-api-lb-target-group"
  port        = 80
  protocol    = "HTTP"
  target_type = "ip"
...
resource "aws_lb_listener" "c7-deleton-api-lb-listener" {
  load_balancer_arn = aws_lb.c7-deleton-api-lb.arn
  port              = 80
  protocol          = "HTTP"
...
resource "aws_lb" "c7-deleton-live-dash-lb" {
  name               = "c7-deleton-live-dash-lb"
  load_balancer_type = "application"
  subnets            = ["subnet-0bd43551b596597e1", "subnet-0b265a90c0cadfb99", "subnet-07f982f51c870f9d1"]
  security_groups    = ["sg-01745c9fa38b8ed68"]
}
# Create Load Balancer target group for Live dashboard
resource "aws_lb_target_group" "c7-deleton-live-dash-lb-target-group" {
  name        = "c7-deleton-livedash-target-group"
  port        = 80
  protocol    = "HTTP"
  target_type = "ip"
  vpc_id      = data.aws_vpc.c7-vpc.id
...
resource "aws_lb_listener" "c7-deleton-live-dash-lb-listener" {
  load_balancer_arn = aws_lb.c7-deleton-live-dash-lb.arn
  port              = 80
  protocol          = "HTTP"
